...
void *taskIreceive(void *arg){
	...
	*/
	...
	struct matrix dsm;
	   ...
	   //dsm = NULL;
	dsm.id = atoi(saveID);
          //int dsmData[itoi(saveCols)][itoi(saveCols)];
	  //dsm.dsm= dsmData;
	dsm.cols = atoi(saveCols);
	   //dsm.dsm = (int*)malloc(sizeof(int)* dsm.cols);
	   //dsm.dsm = Matrix_create(dsm.cols, dsm.cols);
   	   //free(dsm.dsm);
	
	//int a = 3;
	//Matrix *m1 = (Matrix)Matrix_create(a,a);
 	int **hh;
	//hh[0][0]=1;	
	...
	{	
        ...
        for(i=0; i<dsm.cols;i++)
	{
	    for(j=0;j<dsm.cols;j++)
	    {
		mm[i][j]= saveDSM[i*j+j];
